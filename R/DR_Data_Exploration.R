...
all <- rbindlist(list(ctrp, gdsc1, gdsc2))
...
ggplot(all, aes(x = area_above_curve, colour = Dataset)) +
  ...
  annotate(x=mean(ctrp$area_above_curve), y=30000,label="CTRPv2 Mean",vjust=1.5,geom="text", angle = 90) + 
  annotate(x=median(ctrp$area_above_curve), y=30000,label="CTRPv2 Median",vjust=1.5,geom="text", angle = 90) + 
...
p_targets <- ggplot(lds[,2], aes(x=targets)) +
  geom_freqpoly(bins=100) +
