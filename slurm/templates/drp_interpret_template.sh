...
# python3 -u DRP/src/drp_full_model.py --machine mist --train_file ${TRAIN_FILE} --optimize 0 --train --max_num_epochs 100 --n_folds ${N_FOLDS}  --data_types ${DATA_TYPES} --name_tag ${NAME_TAG} --cv_subset_type ${SUBSET_TYPE} --stratify ${STRATIFY} --full ${FULL} --merge_method ${MERGE_METHOD} --loss_type ${LOSS_TYPE} --omic_standardize --resume
python3 DRP/src/drp_interpretation.py --machine mist --data_types ${DATA_TYPES} --name_tag ${NAME_TAG} --full ${FULL} --min_target 0.8
