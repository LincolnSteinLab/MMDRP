# This script contains an auto-encoder for protein data either with an optional auxiliary tumor classifier.
...
class ProtAutoEncoder(nn.Module):
    def __init__(self, input_dim, tied=True):
        ...
        self.encoder = CustomCoder(input_size=input_dim, first_layer_size=2048, code_layer_size=512,
                                   num_layers=3, encode=True)
        self.decoder = CustomCoder(input_size=input_dim, first_layer_size=2048, code_layer_size=512,
...
if __name__ == '__main__':
    ...
    prot_file_name = 'DepMap_20Q2_No_NA_ProteinQuant.csv'
    tum_file_name = 'DepMap_20Q2_Line_Info.csv'

    training_set = ProtData(path=PATH, prot_file_name=prot_file_name)
    training_loader = data.DataLoader(training_set, batch_size=batch_size, num_workers=2, shuffle=True)

    cur_model = ProtAutoEncoder(input_dim=training_set.width(),
                                tied=True)
    ...
    if action == "continue":
        ...
        model_path = 'prot_embed_pytorch.pth.tar'
        checkpoint = torch.load(model_path, map_location=device)
        cur_model.load_state_dict(checkpoint['state_dict'])
    ...
    for epoch in range(last_epoch, num_epochs):
        ...
        batch_start = time.time()
        for i, local_batch in enumerate(training_loader, 0):
            cur_prot = local_batch
            # print("Batch Shape: %3d %3d", cur_prot.shape[0], cur_prot.shape[1])
            ...
            # Transfer to GPU
            # local_batch = local_batch
            cur_prot = cur_prot.float()
            cur_prot = cur_prot.to(device)
            ...
            output = cur_model(cur_prot)
            # loss = categorical_cross_entropy(outputs, local_batch)
            loss = criterion(output, cur_prot)
