# This script contains an auto-encoder for CGC filtered mutational data either with an optional auxiliary tumor tissue
# classifier.
...
from DRP.src.DataImportModules import MutData
...
class MutAutoEncoder(nn.Module):
    def __init__(self, input_dim, tied=True):
        super(MutAutoEncoder, self).__init__()
        self.encoder = CustomCoder(input_size=input_dim, first_layer_size=512, code_layer_size=128,
                                   num_layers=3, encode=True)
        self.decoder = CustomCoder(input_size=input_dim, first_layer_size=512, code_layer_size=128,
...
if __name__ == '__main__':
    ...
    mut_file_name = 'DepMap_20Q2_CGC_Mutations_by_Cell.csv'
    tum_file_name = 'DepMap_20Q2_Line_Info.csv'

    training_set = MutData(path=PATH, mut_file_name=mut_file_name,
                           tum_file_name=tum_file_name)
    training_loader = data.DataLoader(training_set, batch_size=batch_size, num_workers=2, shuffle=True)

    cur_model = MutAutoEncoder(input_dim=training_set.width(),
                               tied=True)
    ...
    if action == "continue":
        ...
        model_path = 'mut_embed_pytorch.pth.tar'
        checkpoint = torch.load(model_path, map_location=device)
        cur_model.load_state_dict(checkpoint['state_dict'])
    ...
    for epoch in range(last_epoch, num_epochs):
        ...
        batch_start = time.time()
        for i, local_batch in enumerate(training_loader, 0):
            cur_mut = local_batch[0]
            # print("Batch Shape: %3d %3d", cur_mut.shape[0], cur_mut.shape[1])
            ...
            # Transfer to GPU
            # local_batch = local_batch
            cur_mut = cur_mut.float()
            cur_mut = cur_mut.to(device)
            ...
            output1 = cur_model(cur_mut)
            # loss = categorical_cross_entropy(outputs, local_batch)
            loss1 = criterion1(output1, cur_mut)
